...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Mutex protocol")
	...
	log.Info("Mutex protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Mutex peer connected", "name", p.Name())

	// Execute the Mutex handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkid.NewID(pm.blockchain), pm.forkFilter); err != nil {
		p.Log().Debug("Mutex handshake failed", "err", err)
		return err
	}
